...
#manager test / 2017.10.30 21:08 FM
...
#sudo /home/nfv/openNetVM/onvm/onvm_mgr/build/onvm_mgr -l $cpu -n 4 --proc-type=primary ${virt_addr} -- -p ${ports} ${num_srvc} ${def_srvc} ${stats} ${stats_sleep_time}
if [ "${stats}" = "-s web" ]
then
    kill $ONVM_WEB_PID
fi
